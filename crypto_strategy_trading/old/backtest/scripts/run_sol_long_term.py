...
"""
📈 SOL 长期回测脚本 (Long-term SOL Backtest)

目的:
    在 SOLUSDT 上运行原始南瓜汤策略，覆盖 2023-2024 年数据。
    验证策略在长周期内的表现。
"""
...
def run_long_term_backtest():
    ...
    file_path = data_dir / "SOLUSDT-1h-merged.csv"
    ...
    strategy = PumpkinSoupStrategy(
        parameters={
            ...
            'risk_per_trade': 0.02,
            # 恢复默认过滤器设置
            'enable_mtf_filter': True,  # 开启多周期共振
            'enable_chop_filter': False, # 默认关闭震荡过滤
            'enable_adx_filter': False,  # 默认关闭ADX
            'ema_spread_threshold': 0.015, # 恢复默认 1.5% 阈值
        }
    )
    
    # 3. 运行回测
    logger.info("开始回测 (2023-2024)...")
